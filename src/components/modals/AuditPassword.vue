<template>
    <div class="fixed inset-0 bg-black/55 flex items-center justify-center z-50">
        <div class="bg-stone-900 border border-white/[0.08] rounded-2xl p-7 w-full max-w-md shadow-2xl">
            ...
            <DashboardAuditPassword v-if="audited" :password="password" :audit-results="auditResults" :breachCount="breachCount"/>
        </div>
    </div>
</template>
...
import DashboardAuditPassword from '../DashboardAuditPassword.vue';
import { rateDiversity, rateEntropy, rateLength, checkAllPatterns, checkBreach } from '@/services/passwordService.ts'

export default {
    emits: ['close'],
    components: { DashboardAuditPassword },
    data() {
        return {
            password: '',
            loading: false,
    ...
    methods: {
        async auditar() {
            ...
            this.breachCount = await checkBreach(this.password)
             
            this.auditResults = {
                length:    rateLength(this.password),
                diversity: rateDiversity(this.password),
                entropy:   rateEntropy(this.password),
                patterns:  checkAllPatterns(this.password) ? 'low' : 'high',
                breaches:  this.breachCount > 100 ? 'low' : this.breachCount > 0 ? 'mid' : 'high',
            }
            this.loading = false
            this.audited = true
        }
