    ...
    <div class="fixed inset-0 bg-black/55 flex items-center justify-center z-50">
        <div class="bg-stone-900 border border-white/[0.08] rounded-2xl p-7 w-full max-w-md shadow-2xl">
            ...
            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-indigo-500/15 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-[17px] h-[17px] text-indigo-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M12 3a9 9 0 0 0-9 9v.6l2.8 5.4A2 2 0 0 0 7.6 19H9v-7h6v7h1.4a2 2 0 0 0 1.8-1l2.8-5.4V12a9 9 0 0 0-9-9z" />
                        </svg>
                    </div>
                    <h2 class="text-stone-100 text-base font-medium">Verificación de seguridad</h2>
                </div>
                <button class="w-7 h-7 rounded-md border border-white/10 text-stone-400 hover:text-stone-200 hover:bg-white/5 flex items-center justify-center text-sm transition" @click="$emit('abort')" aria-label="Cerrar">✕</button>
            </div>
            ...
            <form @submit.prevent="verifyPassword">
                <div class="mb-5">
                    <label for="password" class="block text-xs font-medium text-stone-300 uppercase tracking-widest mb-2">
                        Contraseña
                    </label>
                    <div class="relative">
                        ...
                        <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-stone-500 hover:text-stone-300 transition"
                            @click="showPassword = !showPassword" aria-label="Mostrar contraseña">
                            <svg v-if="!showPassword" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
